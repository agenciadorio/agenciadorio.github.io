...
class WCFM_Product_Popup {
					...
					if( apply_filters( 'wcfm_is_allow_add_products', true ) ) {
						if( apply_filters( 'wcfm_is_allow_product_popup', true ) ) {
							if( apply_filters( 'wcfm_is_allow_product_limit', true ) ) {
								...
								add_action( 'wcfm_load_scripts', array( &$this, 'wcfm_product_popup_load_scripts' ), 30 );
								add_action( 'after_wcfm_load_scripts', array( &$this, 'wcfm_product_popup_load_scripts' ), 30 );
								
								// Product Popup Load WCFM Styles
								add_action( 'wcfm_load_styles', array( &$this, 'wcfm_product_popup_load_styles' ), 30 );
								...
								add_action( 'before_wcfm_load_views', array( &$this, 'wcfm_product_popup_load_views' ), 300 );
  ...
  public function wcfm_product_popup_load_views( $end_point ) {
	  ...
	  if( !in_array( $end_point, $wcfm_blocked_product_popup_views ) ) {
      //require_once( $WCFM->library->views_path . 'products-popup/wcfm-view-product-popup.php' );
    }
  }
	...
	function wcfm_product_popup_botton( $end_point ) {
		...
		if( !in_array( $end_point, $wcfm_blocked_product_popup_views ) ) {
			require_once( $WCFM->library->views_path . 'products-popup/wcfm-view-product-popup-button.php' );
			require_once( $WCFM->library->views_path . 'products-popup/wcfm-view-product-popup.php' );
		}
	}
}