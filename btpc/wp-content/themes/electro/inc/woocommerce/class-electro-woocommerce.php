	...
	class Electro_WooCommerce {
		...
		public function modify_product_tabs( $tabs ) {
			...
			if ( isset( $tabs['reviews'] ) ) {
				$tabs['reviews']['title'] = esc_html__( 'Avaliações', 'electro' );
			}

			if ( isset( $tabs['additional_information'] ) ) {
				$tabs['additional_information']['title'] = esc_html__( 'Especificações', 'electro' );
			}
			...
			if ( $product && ( !empty( $product->specifications ) || ( $product->specifications_display_attributes == 'yes' && ( $product->has_attributes() || ( $product->enable_dimensions_display() && ( $product->has_dimensions() || $product->has_weight() ) ) ) ) ) ) {
				$tabs['specification'] = array(
					'title'    => esc_html__( 'Especificações', 'electro' ),
					'priority' => 20,
