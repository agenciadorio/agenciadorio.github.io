			...
			foreach($wcfm_products_array as $wcfm_products_single) {
				...
				if( !$WCFM->is_marketplace || wcfm_is_vendor() ) {
					...
					if( $WCFM->is_marketplace == 'wcmarketplace' ) {
						...
						$vendor_user = get_user_by( 'id', $wcfm_products_single->post_author );
				...
				if( $wcfm_products_single->post_status == 'publish' ) {
					...
					$actions .= ( apply_filters( 'wcfm_is_allow_edit_specific_products', true, $wcfm_products_single->ID ) ) ? '<a class="wcfm-action-icon" href="' . get_wcfm_edit_product_url($wcfm_products_single->ID, $the_product) . '"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', 'wc-frontend-manager' ) . '"></span></a>' : '';
