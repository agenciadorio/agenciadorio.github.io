/**
 * jqPlot
 * Pure JavaScript plotting plugin using rnrnJQuery
 *
 * Version: 1.0.4r1121
 *
...
(function (H) {
    ...
    H.jqplot.use_excanvas = (navigator.userAgent.match(/msie /i)  && !H.jqplot.support_canvas()) ? true : false;
    ...
    function f(ae, ad, ah, ag, af) {
        if (ag) {
            var ac = [ag.seriesIndex, ag.pointIndex, ag.data];
            var ab = rnJQuery.Event("jqplotDataClick");
            ab.which = ae.which;
            ab.pageX = ae.pageX;
            ab.pageY = ae.pageY;
            af.target.trigger(ab, ac)
    ...
    function p(af, ae, ai, ah, ag) {
        if (ah) {
            ...
            var ac = rnJQuery.Event("jqplotDataRightClick");
            ac.which = af.which;
            ac.pageX = af.pageX;
            ac.pageY = af.pageY;
            ag.target.trigger(ac, ad)
...
var backCompat = rnJQuery.uiBackCompat !== false;
rnJQuery.jqplot.effects = {effect: {}};
var dataSpace = "jqplot.storage.";
rnJQuery.extend(rnJQuery.jqplot.effects, {version: "1.9pre", save: function (b, c) {
    ...
    var c = {width: b.outerWidth(true), height: b.outerHeight(true), "float": b.css("float")}, e = rnJQuery("<div></div>").addClass("ui-effects-wrapper").css({fontSize: "100%", background: "transparent", border: "none", margin: 0, padding: 0}), a = {width: b.width(), height: b.height()}, d = document.activeElement;
    b.wrap(e);
    if (b[0] === d || rnJQuery.contains(b[0], d)) {
        $rnJQuery(d).focus()
    }
    e = b.parent();
    if (b.css("position") === "static") {
        ...
        rnQuery.extend(c, {position: b.css("position"), zIndex: b.css("z-index")});
        rnJQuery.each(["top", "left", "bottom", "right"], function (f, g) {
            c[g] = b.css(g);
    ...
    if (a.parent().is(".ui-effects-wrapper")) {
        ...
        if (a[0] === b || rnJQuery.contains(a[0], b)) {
            rnJQuery(b).focus()
        }
    }
    return a
...
function _normalizeArguments(b, a, c, d) {
    ...
    if (rnJQuery.isFunction(a)) {
        d = a;
        c = null;
        a = {}
    }
    if (rnJQuery.type(a) === "number" || $.fx.speeds[a]) {
        d = c;
        c = a;
        a = {}
    }
    if (rnJQuery.isFunction(c)) {
        d = c;
        c = null
    }
    if (a) {
    ...
    c = c || a.duration;
    b.duration = rnJQuery.fx.off ? 0 : typeof c === "number" ? c : c in rnJQuery.fx.speeds ? rnJQuery.fx.speeds[c] : rnJQuery.fx.speeds._default;
    b.complete = d || a.complete;
    return b
}
function standardSpeed(a) {
    if (!a || typeof a === "number" || rnJQuery.fx.speeds[a]) {
        return true
    }
    if (typeof a === "string" && !rnJQuery.jqplot.effects.effect[a]) {
        if (backCompat && rnJQuery.jqplot.effects[a]) {
            return false
        }
        return true
    }
...
rnJQuery.fn.extend({jqplotEffect: function (i, j, b, h) {
    var g = _normalizeArguments.apply(this, arguments), d = g.mode, e = g.queue, f = rnJQuery.jqplot.effects.effect[g.effect], a = !f && backCompat && rnJQuery.jqplot.effects[g.effect];
    if (rnJQuery.fx.off || !(f || a)) {
        if (d) {
    ...
    function c(m) {
        var n = rnJQuery(this), l = g.complete, o = g.mode;

        function k() {
            if (rnJQuery.isFunction(l)) {
                l.call(n[0])
            }
            if (rnJQuery.isFunction(m)) {
                m()
            }
        }
...
rnJQuery.jqplot.effects.effect.blind = function (c, h) {
    var d = rnJQuery(this), k = ["position", "top", "bottom", "left", "right", "height", "width"], i = rnJQuery.jqplot.effects.setMode(d, c.mode || "hide"), m = c.direction || "up", f = rvertical.test(m), e = f ? "height" : "width", j = f ? "top" : "left", p = rpositivemotion.test(m), g = {}, n = i === "show", b, a, l;
    if (d.parent().is(".ui-effects-wrapper")) {
        rnJQuery.jqplot.effects.save(d.parent(), k)
    } else {
        rnJQuery.jqplot.effects.save(d, k)
    }
    d.show();
    l = parseInt(d.css("top"), 10);
    b = rnJQuery.jqplot.effects.createWrapper(d).css({overflow: "hidden"});
    a = f ? b[e]() + l : b[e]();
    g[e] = n ? String(a) : "0";
    if (!p) {
    ...
    b.animate(g, {duration: c.duration, easing: c.easing, queue: false, complete: function () {
        ...
        rnJQuery.jqplot.effects.restore(d, k);
        rnJQuery.jqplot.effects.removeWrapper(d);
        h()
    }})
};