	...
	class MultiPostThumbnails {
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			if (version_compare($wp_version, '3.5', '<')) {	
				...
				wp_enqueue_media();
				$template_url = get_template_directory_uri() . '/js/multi-post-thumbnails-admin.js';
				$template_url2 = get_template_directory_uri() . '/js/media-modal.js';
				wp_enqueue_script("mpt-featured-image",  $template_url, array('jquery','set-post-thumbnail'));
				wp_enqueue_script( "mpt-featured-image-modal", $template_url2, array( 'jquery', 'media-models' ) );				
			}
		}
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			if ($this->post_type=="rooms") { $format_string = '<p class="hide-if-no-js"></b></p><a title="%1$s" href="%2$s" id="set-%3$s-%4$s-thumbnail" class="%5$s" data-thumbnail_id="%7$s" data-uploader_title="%1$s" data-uploader_button_text="%1$s">%%s</a><p><b>Note:</b> this image will shows only on individual room page.</p></p>'; }
			
			if ($this->post_type=="post") { $format_string = '<p class="hide-if-no-js"></b></p><a title="%1$s" href="%2$s" id="set-%3$s-%4$s-thumbnail" class="%5$s" data-thumbnail_id="%7$s" data-uploader_title="%1$s" data-uploader_button_text="%1$s">%%s</a><p><b>Note:</b> thumbnail will shows only in sidebar widgets e.g. Popular Posts and Our Latest News on main page. If you don\'t set it cropped version of featured image will used instead.</p></p>'; }
