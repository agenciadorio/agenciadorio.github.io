...
function rednao_smart_forms_load_form($title,$form_id,$returnComponent)
{
    ...
    if($returnComponent==false)
    {
        ...
        <div id="formContainer<?php echo $random?>" class='sfForm rednaoFormContainer bootstrap-wrapper SfFormElementContainer'></div>
        <script type="text/javascript">
			var smartFormsCurrentTime=new Date("<?php echo date('D M d Y H:i:s O')?>");
            var smartFormsUserName="<?php echo $userName?>";
            var smartFormsFirstName="<?php echo $firstName?>";
            var smartFormsLastName="<?php echo $lastName?>";
            var smartFormsEmail="<?php echo $email?>";
			...
			var smartFormsAdditionalFields<?php echo ($options['form_id']==null?0:$options['form_id'])?>=<?php echo json_encode($additionalFields)?>;
            if(!window.smartFormsItemsToLoad)
                window.smartFormsItemsToLoad=[];

            window.smartFormsItemsToLoad.push({'form_id':<?php echo $options['form_id']?>, 'elements':<?php echo str_replace("\xe2\x80\xa8", '\\u2028',$options['elements'])?>,'client_form_options':<?php echo str_replace("\xe2\x80\xa8", '\\u2028',$options['client_form_options'])?>,'container':'formContainer<?php echo $random?>'});
        ...
        $script="
            ...
            var smartFormsCurrentTime=new Date('".date('D M d Y H:i:s O')."');
                ...
                var smartFormsAdditionalFields".($options['form_id']==null?0:$options['form_id'])."=".json_encode($additionalFields).";
                var smartFormsDesignMode=false;
                if(!window.smartFormsItemsToLoad)
...
function rednao_smart_forms_load_preview(){
    ...
    if($previewPage==null)
    {
        ...
        wp_redirect( trailingslashit( site_url() ) . '?page_id='.$previewPage->ID.'&sfpreview=true');
