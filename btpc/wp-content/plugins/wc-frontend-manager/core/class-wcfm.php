...
class WCFM {
	...
	function init() {
		...
		if( !is_admin() && ( WCFM_Dependencies::wcfm_wc_dhl_shipping_active_check() || WCFM_Dependencies::wcfm_wc_fedex_shipping_active_check() ) ) {
			require_once( $this->library->views_path . 'orders/wcfm-view-orders-details-fedex-dhl-express.php' );
		}
	...
	static function update_wcfm() {
		...
		$wcfm_tables = $wpdb->query( "SHOW tables like '{$wpdb->prefix}wcfm_enquiries_meta'");
		if( !$wcfm_tables ) {
			delete_option( 'wcfm_updated_4_1_3' );
			delete_option( 'wcfm_table_install' );
		}
		if( !get_option( 'wcfm_updated_4_1_3' ) ) {
			require_once ( $WCFM->plugin_path . 'helpers/class-wcfm-install.php' );
			$WCFM_Install = new WCFM_Install();
			update_option( 'wcfm_updated_4_1_3', 1 );
		}
