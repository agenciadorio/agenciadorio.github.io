...
class WCFM_Vendor_Support {
		...
		if( $is_marketplace = wcfm_is_marketplace() ) {
			if ( !is_admin() || defined('DOING_AJAX') ) {
				...
				if( $is_marketplace == 'wcmarketplace' ) {
					...
					// WCMp Policies Tab
					if ( apply_filters( 'wcmp_vendor_can_overwrite_policies', true ) && apply_filters( 'wcfm_is_allow_policies', true )  ) {
						add_action( 'end_wcfm_products_manage', array( &$this, 'wcmarketplace_product_policies' ), 490 );
						add_action( 'after_wcfm_products_manage_meta_save', array( &$this, 'wcmarketplace_product_policies_save' ), 490, 2 );
					}
				}
  ...
  function vendors_wcfm_menus( $menus ) {
	...
	// WCMp Product Policied
	function wcmarketplace_product_policies( $product_id ) {
		global $WCFM, $WCMp;
		
		$_wcmp_cancallation_policy = get_wcmp_vendor_settings('cancellation_policy');
		$_wcmp_refund_policy = get_wcmp_vendor_settings('refund_policy');
		$_wcmp_shipping_policy = get_wcmp_vendor_settings('shipping_policy');
		
		if( $product_id  ) {
			$_wcmp_cancallation_policy = get_post_meta( $product_id, '_wcmp_cancallation_policy', true);
			...
			$_wcmp_shipping_policy = get_user_meta( $vendor_id, '_vendor_shipping_policy', true) ? get_user_meta( $vendor_id, '_vendor_shipping_policy', true) : $_wcmp_shipping_policy;
			$_wcmp_refund_policy = get_user_meta( $vendor_id, '_vendor_refund_policy', true) ? get_user_meta( $vendor_id, '_vendor_refund_policy', true) : $_wcmp_refund_policy;
			$_wcmp_cancallation_policy = get_user_meta( $vendor_id, '_vendor_cancellation_policy', true) ? get_user_meta( $vendor_id, '_vendor_cancellation_policy', true) : $_wcmp_cancallation_policy;
		}
		?>
		<!-- collapsible - WCMp Policies -->
		<div class="page_collapsible products_manage_policies simple variable grouped external booking" id="wcfm_products_manage_form_policies_head"><label class="fa fa-ambulance"></label><?php _e('Policies', 'wc-frontend-manager'); ?><span></span></div>
		<div class="wcfm-container simple variable external grouped booking">
			<div id="wcfm_products_manage_form_policies_expander" class="wcfm-content">
				...
				$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'product_manage_fields_policies', array(  
																																														"_wcmp_shipping_policy" => array('label' => __('Shipping Policy', 'dc-woocommerce-multi-vendor'), 'type' => $wpeditor, 'class' => 'wcfm-textarea wcfm_ele simple variable external grouped booking wcfm_custom_field_editor ' . $rich_editor, 'label_class' => 'wcfm_title', 'value' => $_wcmp_shipping_policy ),
																																														"_wcmp_refund_policy" => array('label' => __('Refund Policy', 'dc-woocommerce-multi-vendor'), 'type' => $wpeditor, 'class' => 'wcfm-textarea wcfm_ele simple variable external grouped booking wcfm_custom_field_editor ' . $rich_editor, 'label_class' => 'wcfm_title', 'value' => $_wcmp_refund_policy ),
																																														"_wcmp_cancallation_policy" => array('label' => __('Cancellation/Return/Exchange Policy', 'dc-woocommerce-multi-vendor'), 'type' => $wpeditor, 'class' => 'wcfm-textarea wcfm_ele simple variable external grouped booking wcfm_custom_field_editor ' . $rich_editor, 'label_class' => 'wcfm_title', 'value' => $_wcmp_cancallation_policy ),
																																									), $product_id ) );
				?>
			</div>
		</div>
	...
	// WCMp Product Policies Save
	function wcmarketplace_product_policies_save( $new_product_id, $wcfm_products_manage_form_data ) {
		global $WCFM, $WCMp;
		
		if( isset( $wcfm_products_manage_form_data['_wcmp_shipping_policy'] ) && !empty( $wcfm_products_manage_form_data['_wcmp_shipping_policy'] ) ) {
			update_post_meta( $new_product_id, '_wcmp_shipping_policy', $wcfm_products_manage_form_data['_wcmp_shipping_policy'] );
		}
		if( isset( $wcfm_products_manage_form_data['_wcmp_refund_policy'] ) && !empty( $wcfm_products_manage_form_data['_wcmp_refund_policy'] ) ) {
			update_post_meta( $new_product_id, '_wcmp_refund_policy', $wcfm_products_manage_form_data['_wcmp_refund_policy'] );
		}
		if( isset( $wcfm_products_manage_form_data['_wcmp_cancallation_policy'] ) && !empty( $wcfm_products_manage_form_data['_wcmp_cancallation_policy'] ) ) {
			update_post_meta( $new_product_id, '_wcmp_cancallation_policy', $wcfm_products_manage_form_data['_wcmp_cancallation_policy'] );
		}
	}
		...
		if( $is_marketplace ) {
			if( !wcfm_is_vendor() ) {
				...
				if( $is_marketplace == 'wcpvendors' ) {
					...
					$args = array(
						'role__in'     => apply_filters( 'wcfm_allwoed_user_rols', array( 'dc_vendor', 'vendor', 'seller', 'disable_vendor' ) ),
						'orderby'      => 'login',
						'order'        => 'ASC',
  ...
  function wcfm_admin_notification_new_product( $vendor_id, $product_id ) {
  	...
  	if( !$wcfm_new_product_notified ) {
			...
			$WCFM->frontend->wcfm_send_direct_message( $author_id, $message_to, $author_is_admin, $author_is_vendor, $wcfm_messages, 'new_product', apply_filters( 'wcfm_is_allow_new_product_notification_email', false ) );
  ...
  function wcfm_admin_notification_product_review( $vendor_id, $product_id ) {
  	...
  	if( !$wcfm_review_product_notified ) {
			...
			$WCFM->frontend->wcfm_send_direct_message( $author_id, $message_to, $author_is_admin, $author_is_vendor, $wcfm_messages, 'product_review', apply_filters( 'wcfm_is_allow_review_product_notification_email', true ) );
				
			update_post_meta( $product_id, '_wcfm_review_product_notified', true );
